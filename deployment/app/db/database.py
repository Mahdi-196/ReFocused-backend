...
masked_db_url_start = _mask_url_credentials(settings.DATABASE_URL)
logger.info(f"🌐 DATABASE_URL (masked): {masked_db_url_start}")
...
masked_db_url_sanitized = _mask_url_credentials(sanitized_database_url)
if masked_db_url_sanitized != masked_db_url_start:
    logger.info(f"🌐 DATABASE_URL (sanitized masked): {masked_db_url_sanitized}")
...
async def get_db():
    ...
    if not _db_preflight_ran:
        ...
        # Temporary: allow disabling DB preflight via env (default: disabled)
        preflight_enabled = os.getenv("DB_PREFLIGHT_ENABLED", "0").lower() in ("1", "true", "yes", "on")
        if preflight_enabled:
            # Run preflight without blocking too long
            try:
                await _run_db_preflight(sanitized_database_url)
            except Exception:
                pass
        else:
